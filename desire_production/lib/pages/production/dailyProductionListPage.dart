...
class DailyProductionListPage extends StatefulWidget {
  ...
  @override
  _DailyProductionListPageState createState() => _DailyProductionListPageState();
}
...
class _DailyProductionListPageState extends State<DailyProductionListPage> {
  ...
  List<bool> check = [];
  bool checkAll = false;
  final DailyProductionListBloc dailyProductionListBloc = DailyProductionListBloc();
  List<String> send = [];
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          ...
          actions: [
            PopupMenuButton(
                ...
                itemBuilder: (b) =>[
                  PopupMenuItem(
                      child: TextButton(
                        child: Text("Generate PDF", textAlign: TextAlign.center,style: TextStyle(color: Colors.black),),
                        onPressed: (){
                          print("object search result inside pdf ${_searchResult.length}");
                          Navigator.pop(context);
                          _searchResult.length == 0 ? generatePdf() : generatePdfSearch();
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          }, label: Text("Add Model"),icon: Icon(Icons.add,color: Colors.white  ,),),
        body: _body(),
      ),
    );
  }
  ...
  Widget _body(){
    return RefreshIndicator(
      color: kPrimaryColor,
      onRefresh: (){
        return Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (builder) => DailyProductionListPage(page: widget.page,)), (route) => false);
      },
                                        ...
                                        Expanded(
                                            child: GestureDetector(
                                              onTap: (){
                                                ...
                                                => DailyProductionListByModelNumber(modelNoId: s.data.data[i].modelNoId,
                                                  modelNo: s.data.data[i].modelNo,
                                                  dailyProductionId: s.data.data[i].dailyProductionId,
                                                )
                                                ));
                                              },
                                              child: Container(
