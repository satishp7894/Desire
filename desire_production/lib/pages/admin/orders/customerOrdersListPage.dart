...
class _CustomerOrdersListPageState extends State<CustomerOrdersListPage> {
  ...
  void initState() {
    ...
    allCustomerOrderBloc.fetchAllCustomerOrders();
  }
    ...
    return RefreshIndicator(
      ...
      onRefresh: () {
        return Navigator.pushReplacement(context,
            MaterialPageRoute(builder: (builder) => CustomerOrdersListPage()));
      },
      child: StreamBuilder<CustomerOrdersModel>(
          ...
          builder: (c, s) {
            if (s.connectionState != ConnectionState.active) {
              ...
              asyncSnapshot = s;
              return asyncSnapshot.data.data != null &&
                      asyncSnapshot.data.data.length != 0
                  ? ListView.builder(
                      shrinkWrap: true,
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    "Order Number : " +
                                        asyncSnapshot.data.data[i].orderNumber,
                                    style: TextStyle(
                                        fontSize: 14,
                                        fontWeight: FontWeight.bold,
                                        color: kPrimaryColor),
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Text(
                                        "Quantity : ${asyncSnapshot.data.data[i].totalOrderQuantity}",
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold),
                                      ),
                                      Text(
                                        "Ordered on : " +
                                            "${asyncSnapshot.data.data[i].orderDate.split(" ").first}",
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 14),
                                      ),
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Text(
                                        "Total Amount : ${asyncSnapshot.data.data[i].orderAmount}",
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 14),
                                      ),
  ...
  sendToDailyOrder(String customerId, String orderId) async {
    var response = await http.post(
        Uri.parse(
            "http://loccon.in/desiremoulding/api/SalesApiController/sendToDailyOrder"),
        body: {
