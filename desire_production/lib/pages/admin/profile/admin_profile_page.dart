...
class _AdminProfilePageState extends State<AdminProfilePage> with Validator {
  ...
  getDetails() async {
    ...
    name.text = prefs.getString("fname");
    lname.text = prefs.getString("lname");
    email.text = prefs.getString("email");
    phone.text = prefs.getString("mobile");
    uname.text = prefs.getString("username");
    password = prefs.getString("show_password");
    passwordOld = prefs.getString("show_password");
    ...
    print("pass $passwordOld $password ${prefs.getString("show_password")}");
  }
                        ...
                        children: <Widget>[
                          new Flexible(
                            child: TextFormField(
                              ...
                              onEditingComplete: () {
                                if (md5.convert(utf8.encode(pass.text))
                                    .toString() != password) {
                                  Alerts.showAlertAndBack(
                                ...
                                print(
                                    "object value of pass inside on edit complete $password $passwordOld");
                              },
                              enabled: !_status1,
                              autofocus: !_status1,
  ...
  getUpdated() async {
    if (_formkey.currentState.validate()) {
      ...
      var response = await http.post(
          Uri.parse(Connection.updateProfile), body: {
      ...
      if (results['status'] == true) {
        ...
        Navigator.pushAndRemoveUntil(context,
            MaterialPageRoute(builder: (context) => AdminProfilePage()), (Route<dynamic> route) => false);
      } else {
  ...
  getOtherAttributes(String pin) async {
    ...
    if (pin.isEmpty || pin.length > 6) {
      ...
      var response = await http.get(
          Uri.parse(Connection.getAttributes + "$pin"));
  ...
  getLoginUpdated() async {
    ...
    if (_formkey1.currentState.validate()) {
      ...
      pr.style(message: 'Please wait...',
        progressWidget: Center(child: CircularProgressIndicator()),);
      pr.show();
      var response = await http.post(Uri.parse(Connection.changePass), body: {
      ...
      if (results['status'] == true) {
        _status = true;
        Alerts.showLogOut(
            context, "Password Successfully Changed", "Login to Access");
      } else {
        print('error in updating profile');
        Alerts.showAlertAndBack(context, 'Error',
            'Login details not Updated. Please try again later');
      }
    } else {
      setState(() {
