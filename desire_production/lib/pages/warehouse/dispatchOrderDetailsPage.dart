...
class DispatchOrderDetailsPage extends StatefulWidget {
  ...
  const DispatchOrderDetailsPage(
      {Key key, this.orderId, this.customerId, this.page})
      : super(key: key);

  @override
  _DispatchOrderDetailsPageState createState() =>
      _DispatchOrderDetailsPageState();
}

class _DispatchOrderDetailsPageState extends State<DispatchOrderDetailsPage> {
  final DispatchOrderDetailsListBloc dispatchOrderDetailsListBloc =
      DispatchOrderDetailsListBloc();
      ...
      onWillPop: () {
        return widget.page == "warHouse"
            ...
            : Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                    builder: (builder) => ReadyToDispatchListPage(
                          page: 'admin',
                        )),
              );
      },
    ...
    return RefreshIndicator(
        ...
        onRefresh: () {
          return Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(
                  builder: (builder) => DispatchOrderDetailsPage(
            ...
            child: Column(
              children: [
                StreamBuilder<DispatchOrderDetailsModel>(
                    stream:
                        dispatchOrderDetailsListBloc.dispatchOrderDetailsStream,
                    builder: (c, s) {
                      if (s.connectionState != ConnectionState.active) {
                        ...
                        print(
                            "object length ${s.data.data.length} ${check.length}");
                        return _list == null
                            ? Center(
                ...
                ElevatedButton(
                    ...
                    onPressed: () {
                      if (invoiceController.text.isEmpty &&
                          lrNoController.text.isEmpty &&
                          eWayBillViewController.text.isEmpty) {
                        print("Field cant be empty");
                      } else {
                        sendToDispatch(
                            send.toString(), widget.orderId, widget.customerId);
                      }
                    },
                    child: Text("Submit"))
              ],
  ...
  sendToDispatch(String wdId, String orderId, String customerId) async {
    ...
    if (results['status'] == true) {
      ...
      Alerts.showAlertAndBack(
          context, "Success", "Successfully Sent To Dispatch");
    } else {
      Alerts.showAlertAndBack(
        context,
        "Something Went Wrong",
        "Could not send to Dispatch",
      );
    }
  }
