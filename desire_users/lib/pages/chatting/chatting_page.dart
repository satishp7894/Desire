...
class ChattingPage extends StatefulWidget {
  ...
  const ChattingPage(
      {Key key, this.chatPersonName, this.customerId, this.receiverId})
      : super(key: key);
...
class _ChattingPageState extends State<ChattingPage> {
  final CustomerChatDetailsBloc customerChatDetailsBloc =
      CustomerChatDetailsBloc();
  ...
  clearChatApi() async {
    ...
    var response = await http.post(Uri.parse(url),
        body: {"secretkey": r"12!@34#$5%", "conversation_id": conversationId});
      ...
      Navigator.pushReplacement(
          ...
          MaterialPageRoute(
              builder: (context) => ChattingPage(
                    ...
                    chatPersonName: widget.chatPersonName,
                  )));
    } else {
  ...
  newConversion() async {
    var url =
        "http://loccon.in/desiremoulding/api/UserApiController/newConversation";
    ...
    if (result["status"]) {
      setState(() {
        ...
        customerChatDetailsBloc.fetchCustomerChatDetails(result["conversation_id"]);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text(widget.chatPersonName == "Admin"
              ? "${widget.chatPersonName}"
              : "${widget.chatPersonName} Salesman"),
          centerTitle: true,
          actions: [
            PopupMenuButton(
                itemBuilder: (context) => [
          ...
          return asyncSnapshot.connectionState != ConnectionState.active
              ...
              : asyncSnapshot.hasError
                  ...
                  : asyncSnapshot.data == null
                      ? Text("No Chats")
                      : asyncSnapshot.data.customerChat == []
                          ? Center(
                              child: Text(
                              "No Chat",
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...List.generate(
                                        ...
                                        (index) => CustomerChatTiles(
                                              customerChat: asyncSnapshot
                                                  .data.customerChat[index],
  ...
  _sendMessageApi() async {
    var url =
        "http://loccon.in/desiremoulding/api/UserApiController/sendMessage";
    var response = await http.post(Uri.parse(url), body: {
      "secretkey": r"12!@34#$5%",
      "sender_id": "${widget.customerId}",
      "receiver": widget.receiverId == 1 ? "$admin" : "$salesman",
...
class CustomerChatTiles extends StatelessWidget {
  ...
  const CustomerChatTiles({Key key, this.customerChat, this.salesmanId})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Align(
        alignment: (customerChat.senderId == "1"
            ? Alignment.topLeft
            : customerChat.senderId == salesmanId
                ? Alignment.topLeft
                : Alignment.topRight),
        child: Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(10),
            color: (customerChat.senderId == "1"
                ? kSecondaryColor
                : customerChat.senderId == salesmanId
                    ? kSecondaryColor
                    : kPrimaryColor),
