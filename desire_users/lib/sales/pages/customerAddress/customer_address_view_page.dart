...
class CustomerAddressViewPage extends StatefulWidget {
  ...
  CustomerAddressViewPage({@required this.customerId , @required this.customerName, @required this.page, @required this.salesId, @required this.email, @required this.name, this.customer});
...
class _CustomerAddressViewPageState extends State<CustomerAddressViewPage> with Validator{
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () {
        return Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => CustomerAddressViewPage(customerId: widget.customerId,page: widget.page, customerName: widget.customerName,salesId:widget.salesId, name: widget.name, email: widget.email, customer: widget.customer, )), (Route<dynamic> route) => false);
      },
      child: WillPopScope(
        onWillPop: () {
        ...
        child: SafeArea(
            child:Scaffold(
              appBar: AppBar(
                ...
                title: Text("Shipping Address",style: TextStyle(color: Colors.black,fontSize: 18.0),),
                centerTitle: true,
                elevation: 0,
                              ...
                              child: Column(
                                children: [
                                  IconButton(
                                      ...
                                      onPressed: (){
                                        showDialog(context: context, builder: (builder) => EditDialog(data: s.data.data[i], userId: widget.customerId, page: widget.page, context: context,  salesId: widget.salesId, name: widget.name, customerName: widget.customerName, email: widget.email,));
                                      }),
                                  IconButton(
  ...
  addAddress() async{
    if (_formkey.currentState.validate()) {
      ...
      if (results['status'] == true) {
        Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => CustomerAddressViewPage(customerId: widget.customerId, page: widget.page, customerName: widget.customerName,salesId:widget.salesId, name: widget.name, email: widget.email, customer: widget.customer,)), (Route<dynamic> route) => false);
      } else {
  ...
  delAddress(String id) async{
    ...
    pr.hide();
    if (results['status'] == true) {
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => CustomerAddressViewPage(customerId: widget.customerId, page: "cart", customerName: widget.customerName,salesId:widget.salesId, name: widget.name, email: widget.email, customer:  widget.customer,)), (Route<dynamic> route) => false);
    } else {
...
class EditDialog extends StatefulWidget {
  ...
  const EditDialog({@required this.data, @required this.userId, @required this.page, @required this.context, @required this.customerName, @required this.salesId, @required this.name, @required this.email});
          ...
          onPressed: () async{
            ...
            if (_formkey1.currentState.validate()){
              ...
              if (results['status'] == true) {
                ...
                Navigator.pushAndRemoveUntil(contextDialog, MaterialPageRoute(builder: (builder) => CustomerAddressViewPage(customerId: widget.userId, page: widget.page,  salesId: widget.salesId, name: widget.name, customerName: widget.customerName, email: widget.email,)), (route) => false);
              } else {
