...
class AddCustomerKycPage extends StatefulWidget {
  ...
  final String name;
  final String email;
  AddCustomerKycPage({@required this.customerId, @required this.salesId, @required this.email, @required this.name});
    ...
    return SafeArea(
        child: Scaffold(
          body: RefreshIndicator(
            onRefresh: (){
              return Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (builder) => AddCustomerKycPage(name: widget.name, customerId: widget.customerId,  email: widget.email, salesId: widget.salesId,)), (route) => false);
            },
                      ...
                      child: DefaultButton(text: "Submit",press: (){
                        if(check2 == true && (check1 == true || check3 == true) && kyc2.text.length == 12 && ( kyc1.text.length == 15 || kyc3.text.length == 10)){
                          if (_formKey.currentState.validate()) {
                            ...
                            Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (builder) => CustomerListPage( salesId: widget.salesId, email: widget.email, name: widget.name,customerId: widget.customerId,)), (route) => false);
                          } else {
                            setState(() {
