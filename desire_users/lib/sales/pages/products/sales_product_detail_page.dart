...
class SalesProductDetailsPage extends StatefulWidget {
  ...
  @override
  _SalesProductDetailsPageState createState() =>
      _SalesProductDetailsPageState();
}
...
class _SalesProductDetailsPageState extends State<SalesProductDetailsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: CustomAppBar(
        rating: double.parse(widget.product.id),
        press: () {
                ...
                children: [
                  Align(
                    ...
                    child: Padding(
                      ...
                      child: Text(
                        "Price: ₹${widget.product.salesmanprice}",
                        maxLines: 3,
                        style: appbarStyle,
                  ...
                  children: [
                    Padding(
                      padding: EdgeInsets.only(
                          left: 20, right: 20, top: 10, bottom: 10),
                      child: Text(
                        "Profile No.: ${widget.product.profileNo}",
                        maxLines: 3,
                      ),
                    ),
                    Padding(
                      padding: EdgeInsets.only(
                          left: 20, right: 20, top: 10, bottom: 10),
                      child: Text(
                        "Model No.: ${widget.product.modelNo}",
                        maxLines: 3,
                      ),
                    ),
                    Padding(
                      padding: EdgeInsets.only(
                          left: 20, right: 20, top: 10, bottom: 10),
                      child: Text(
                        "Dimensions: ${widget.product.dimensionId}",
                        maxLines: 3,
                      ),
                    ),
                    Padding(
                        ...
                        if (ss.connectionState != ConnectionState.done) {
                          ...
                          if (ss.hasError) {
                            ...
                            return ValueListenableBuilder(
                                valueListenable:
                                    Hive.box(Connection.cartList).listenable(),
                                          ...
                                          child: DefaultButton(
                                            ...
                                            press: () async {
                                              if (id == null) {
                                                ...
                                                cartData['image'] =
                                                    'http://loccon.in/desiremoulding/upload/Image/Product/${products.image[0]}';
                                                cartData['mrp'] = products
                                                    .id; // '${s.data.mrp}';
                                                cartData['qty'] = totQty;
                                                box.put(
                                                    '${products.id}', cartData);
                                                addToCart();
                                              }
                                              // Map cartData = Map();
        ...
        SizedBox(
          ...
          child: AspectRatio(
            ...
            child: Hero(
              ...
              child: Image.network(
                  "http://loccon.in/desiremoulding/upload/Image/Product/" +
  ...
  addToCart() async {
    ...
    if (results['status'] == true) {
      Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(
              builder: (builder) => SalesProductDetailsPage(
                    customerList: widget.customerList,
                    snapshot: widget.snapshot,
...
class _CustomerListDialogState extends State<CustomerListDialog> {
  ...
  void initState() {
    ...
    products = widget.product;
    print(
        "${widget.customerList.length} ${_searchResult.length} $_verticalGroupValue1");
    customerList = widget.customerList;
  ...
  addToCart() async {
    ...
    if (results['status'] == true) {
      ...
      Alerts.showAlertAndBack(contextDialog, "User Not Found",
          "Please enter registered mobile no or email id");
    }
  }
}
