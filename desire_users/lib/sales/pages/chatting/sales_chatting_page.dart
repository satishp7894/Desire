...
class _SalesChattingPageState extends State<SalesChattingPage> {
  ...
  clearChatApi() async {
    var url =
        "http://loccon.in/desiremoulding/api/SalesApiController/clearChat";
    ...
    if (result["message"] == "Chat clear succefully") {
      print("Chat Clear");
      final snackBar = SnackBar(content: Text("All Chat Cleared Successfully"));
  ...
  void initState() {
    ...
    newConversion();
    getConversationId();
    salesChatDetailsBloc.fetchSalesChatDetails(
        widget.conversationId == "" ? conversationId : widget.conversationId);
    timer = Timer.periodic(
        Duration(seconds: 1),
        (Timer t) => salesChatDetailsBloc.fetchSalesChatDetails(
  ...
  newConversion() async {
    var url =
        "http://loccon.in/desiremoulding/api/SalesApiController/newConversation";
    var response = await http.post(Uri.parse(url), body: {
      "secretkey": r"12!@34#$5%",
    ...
    if (result["status"]) {
      ...
      print("Get error for new conversation");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text(widget.receiverName == "Admin"
              ? "${widget.receiverName}"
              : "${widget.receiverName} Customer"),
          centerTitle: true,
          actions: [
            PopupMenuButton(
                itemBuilder: (context) => [
        ...
        body: Stack(
          children: [chatDetailsView(), _sendMessageView()],
        ));
  }

  _sendMessageApi() async {
    var url =
        "http://loccon.in/desiremoulding/api/SalesApiController/sendMessage";
    var response = await http.post(Uri.parse(url), body: {
      "secretkey": r"12!@34#$5%",
      "sender_id": "${widget.senderId}",
      "receiver": widget.chatWith == 1 ? "$admin" : "$customer",
        ...
        builder: (c, s) {
          ...
          return asyncSnapshot.connectionState != ConnectionState.active
              ...
              : asyncSnapshot.hasError
                  ...
                  : asyncSnapshot.data.status == false
                      ? Center(
                          child: Text(
                          "No Chat",
                          style: TextStyle(color: kBlackColor),
                        ))
                      : asyncSnapshot.data.salesChat == null
                          ? Center(
                              child: Text(
                              "No Chat",
                              style: TextStyle(color: kBlackColor),
                          ...
                          : RefreshIndicator(
                              color: kPrimaryColor,
                              onRefresh: () {
                                return salesChatDetailsBloc
                                    .fetchSalesChatDetails(
                              ...
                              child: SingleChildScrollView(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...List.generate(
                                        asyncSnapshot.data.salesChat.length,
                                        (index) => SalesChatTiles(
                                              salesChat: asyncSnapshot
                                                  .data.salesChat[index],
...
class SalesChatTiles extends StatelessWidget {
  ...
  const SalesChatTiles(
      {Key key, this.salesChat, this.customerId, this.salesmanId})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Align(
        alignment: (salesChat.senderId == salesmanId
            ? Alignment.topRight
            : Alignment.topLeft),
        child: Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(10),
            color: (salesChat.senderId == salesmanId
                ? kPrimaryColor
                : kSecondaryColor),
          ),
