...
import 'package:desire_users/sales/bloc/InvoiceSalesDetailBloc.dart';
...
class _InvoiceDetailSalesPageState extends State<InvoiceDetailSalesPage> {
  InvoiceSalesDetailBloc invoiceDetailBloc = InvoiceSalesDetailBloc();
  List<bool> check = [];
  bool checkAll = false;
  ...
  onSearchTextChangedICD(String text) async {
    ...
    _order.forEach((exp) {
      if (exp.modelNoId.contains(text) ||
          exp.productName.toLowerCase().contains(text.toLowerCase())) _searchResult.add(exp);
    });
    ...
    return RefreshIndicator(
      ...
      onRefresh: () {
        return Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(
                builder: (builder) => InvoiceDetailSalesPage(
                      id: widget.id,
                    )),
            (route) => false);
      },
      child: SingleChildScrollView(
        ...
        child: StreamBuilder<InvoiceDetailModel>(
            ...
            builder: (c, s) {
              ...
              if (s.data.invoiceProducts.length == 0) {
                print("as3 empty");
                return Container(
                  height: 300,
                  alignment: Alignment.center,
  ...
  Future savePdf() async {
    if (await _reqPer(Permission.storage)) {
      ...
      File file = File(dir.path + "/DispatchInvoiceList.pdf");
      path = dir.path + "/DispatchInvoiceList.pdf";
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (context) {
        return <pw.Widget>[
          pw.Header(
              level: 5,
              child: pw.Text("Invoice List | Desire Moulding",
